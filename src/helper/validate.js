...
const checkFormSignupClient = (formValues) => {
...
const checkFormSignupAdmin = (formValues) => {
  const schema = Joi.object({
    email: Joi.string()
      .required()
      .pattern(new RegExp("^[A-Z0-9._%+-]+@[A-Z0-9.-]+\\.(com|net|org)$", "i"))
...
const checkFormLogin = (formValues) => {
  const schema = Joi.object({
    email: Joi.string()
      .required()
      .pattern(new RegExp("^[A-Z0-9._%+-]+@[A-Z0-9.-]+\\.(com|net|org)$", "i"))
...
const checkFormCheckout = (formValues) => {
  const schema = Joi.object({
...
module.exports = {
  user: {
    checkFormLogin,
    checkFormSignupClient,
    checkFormCheckout,
  },
  admin: {
    checkFormLogin,
    checkFormSignupAdmin,
  },
};
